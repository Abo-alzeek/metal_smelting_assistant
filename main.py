...
class IronSmeltingAssistant:
    ...
    def create_main_window(self):
        ...
        self.c_combo['values'] = [option[0] for option in self.get_cooling_options()]
        self.c_combo.grid(row=3, column=1, sticky=(tk.W, tk.E), padx=(5, 0), pady=(5, 0))
        self.c_combo.bind('<<ComboboxSelected>>', self.on_cooling_selected)
        if self.c_combo['values']:
            self.c_combo.current(0)
        
        # Store the selected C value
        self.selected_c_value = None
    ...
    def get_cooling_options(self):
        """
        Return the cooling parameter options with a default option
        """
        return [
            ("اختر معامل التبريد", "default"),
            ("تبريد داخل الفرن (تخمير كامل)", "custom"),
            ("تبريد بالماء", "select"),
            ("تبريد بالزيت", "oil")
        ]
    ...
    def on_cooling_selected(self, event=None):
        """Handle cooling parameter selection"""
        selected = self.c_var.get()
        cooling_options = self.get_cooling_options()
        
        for option_text, option_value in cooling_options:
            if option_text == selected:
                if option_value == "default":
                    # Do nothing for default option
                    pass
                elif option_value == "custom":
                    self.show_custom_cooling_popup()
                elif option_value == "select":
                    self.show_cooling_selection_popup()
                elif option_value == "oil":
                    # Use the last option from get_c_options without popup
                    c_options = self.get_c_options()
                    if c_options:
                        self.selected_c_value = c_options[-1][1]  # Last option value
                break
    
    def show_custom_cooling_popup(self):
        """Show popup for custom cooling value input"""
        popup = tk.Toplevel(self.root)
        popup.title("Custom Cooling Value")
        ...
        ttk.Label(frame, text="أدخل درجة سرعة التبريد", font=("Arial", 12, "bold")).pack(pady=(0, 10))
        ...
        def submit():
            try:
                ...
                if value <= 0:
                    raise ValueError("Value must be positive")
                self.selected_c_value = value
                messagebox.showinfo("Success", f"Custom cooling value set to: {value}")
                popup.destroy()
            except ValueError:
        ...
        def cancel():
            popup.destroy()
            self.c_var.set("")  # Reset dropdown
        
        buttons_frame = ttk.Frame(frame)
        buttons_frame.pack(side=tk.BOTTOM)
        
        ttk.Button(buttons_frame, text="Cancel", command=cancel).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(buttons_frame, text="Submit", command=submit).pack(side=tk.LEFT)
        
        # Bind Enter key to submit
        entry.bind('<Return>', lambda e: submit())
    
    def show_cooling_selection_popup(self):
        """Show popup with three buttons for cooling selection"""
        popup = tk.Toplevel(self.root)
        popup.title("Select Cooling Option")
        ...
        ttk.Label(frame, text="Select Cooling Option:", font=("Arial", 12, "bold")).pack(pady=(0, 20))
        
        # Get first three options from get_c_options
        c_options = self.get_c_options()
        if len(c_options) >= 3:
            options_to_show = c_options[:3]
        else:
            options_to_show = c_options
        
        def select_option(option_text, option_value):
            self.selected_c_value = option_value
            messagebox.showinfo("Cooling Parameter", f"Selected: {option_text} (Value: {option_value})")
            popup.destroy()
        ...
        for option_text, option_value in options_to_show:
            btn = ttk.Button(frame, text=option_text, 
                           command=lambda t=option_text, v=option_value: select_option(t, v),
                           width=25)
            btn.pack(pady=5)
        
        # Cancel button
        ttk.Button(frame, text="Cancel", command=cancel).pack(pady=(10, 0))
    ...
    def submit_data(self):
        ...
        # Check if cooling parameter is selected and not default
        if not self.c_var.get() or self.c_var.get() == "Select Cooling Parameter":
            messagebox.showerror("Missing Parameter", 
                               "Please select a cooling parameter")
            return
    ...
    def show_results(self, carbon_value):
        ...
        # Get C value - check if custom value was selected
        if self.selected_c_value is not None:
            c_value = self.selected_c_value
        else:
            # Fallback to dropdown value if no custom selection
